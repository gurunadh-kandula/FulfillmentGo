        ...
        private async void ListView_SwipeEnded(object sender, SwipeEndedEventArgs e)
        {
            ...
            if (e.SwipeOffset >= 360)
            {
                if (e.SwipeDirection.Equals(Syncfusion.ListView.XForms.SwipeDirection.Left))
                {
                    ...
                    //await Navigation.PopAsync();
                    for (int i = 0; i < ApprovedOrdersPage.approvedOrders.OrderCollection.Count; i++)
                    {
                        if (ApprovedOrdersPage.approvedOrders.OrderCollection[i].OrderID.Equals(orderid))
                        {
                            //ListEnhanced.orders.OrderCollection.Add(ApprovedOrdersPage.approvedOrders.OrderCollection[i]);
                            ListEnhanced.orders.OrderCollection.Insert(0, ApprovedOrdersPage.approvedOrders.OrderCollection[i]);
                            ApprovedOrdersPage.approvedOrders.OrderCollection.Remove(ApprovedOrdersPage.approvedOrders.OrderCollection[i]);
